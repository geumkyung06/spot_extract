...
def analyze_instagram():
    ...
    try:
        ...
        post_places = {"name":[], "address":[], "category":[]} # 프론트에 보낼 장소들 (name, address, category(list))
        new_places = [] # db에 새로 저장할 장소들


        # 1. DB에 이미 URL이 있는지 확인
        print("check db have place...")
        check_db_have_place(post_places=post_places, url=url)
        
        
        # 2. DB에 없는 장소들에 대해 캡션 시도
        caption_places = post_places # 함수 내용
        print("check caption...")
        if caption_places:
            # 캡션 우선 확인
            # 캡션 파싱 함수 필요
            # 캡션은 gpt 사용하지 말고 가게명, 주소 하드코딩으로 찾기 > 거의 형태가 같으니까 일단 이렇게 
            caption_places = check_caption_place(caption_places, url)
            
            
        # 이미지 확인(OCR)
        OCR_places = post_places
        print("start OCR...")
        if OCR_places:

            # OCR 처리 후 장소 정보 업데이트
            OCR_places = check_ocr_place(OCR_places, url)
...
@jwt_required()
def delete_my_place(saved_place):
    try:
...
def check_db_have_place(post_places = {"id":[], "name":[], "address":[], "category":[]}, url=""):
    '''
        1. 인스타그램 저장 로직 변경
        db에 이미 url 존재하는지 
...
def check_caption_place(post_places = {"name":[], "address":[], "category":[]}, url=""):
    '''
    캡션에서 장소 추출
    '''
    # 캡션 파싱 함수 필요
...
def check_ocr_place(post_places = {"name":[], "address":[], "category":[]}, url=""):
    '''
    이미지 OCR에서 장소 추출
    '''
    # 이미지 dom으로 저장 후 바로 gpt4o로 ocr > 이미지 저장은 대략 20초 > 3초 내로 줄어듬 > 아마 23초 정도,,,, 일단 함수 먼저 완성
