...
def _download_google_photo(shortcut, photo_reference: str) -> Optional[str]:
    ...
    try:
        ...
        r = requests.get(GOOGLE_PHOTO_URL, params=params, timeout=20)
        r.raise_for_status()
...
def make_queries(name: str, address: str) -> list:
    clean_addr = address.strip() if address else ""

    if not clean_addr:
        return name

    # 1. 띄어쓰기가 없거나 1개뿐인 짧은 단어는 그대로 붙여서 검색 (예: "신라제면 롯데몰점")
    if len(clean_addr.split()) <= 2:
        return f"{name} {clean_addr}"

    # 2. 주소가 긴 경우 (예: "서울 송파구 올림픽로 300 롯데월드몰 6F")
    tokens = clean_addr.split()
    # 시/구 정보만 추출 (예: "서울 송파구")
    short_region = f"{tokens[0]} {tokens[1]}" 
...
def _search_naver_local(list_query: list) -> dict: # 쿼리 ex) 서울(성수) 진사천훠궈 query[0]으로 [위치+가게명, 주소] 로 변경 필요
    ...
    query = make_queries(list_query[0], list_query[1])
...
def _fetch_google_details(name: str, address: str, shortcut) -> dict:
    ...
    query = make_queries(name, address)
    params = {
        "query": query,
        "key": PLACE_API_KEY,
        "language": "ko",
    ...
    try:
        ...
        if data.get("status") == "OK" and data.get("results"):
            ...
            result_data["address"] = best.get("formatted_address", address)
...
def trans_geo(road_mapx, road_mapy) -> Tuple[float, float]:
    try:
        ...
        if x >= 100000000: 
            ...
            gdf = gpd.GeoDataFrame(geometry=gpd.points_from_xy([int(x)], [int(y)]))
            gdf.crs = 'epsg:5179' 
...
def process_places(place_queries: list[str], shortcut) -> list[dict]: # [[name, address], [name, address]...]
    ...
    for idx, query in enumerate(place_queries):
        ...
        lat, lng = 0.0, 0.0
        naver_item = _search_naver_local([orig_name, orig_addr])
        if naver_item :
            # 네이버 데이터 정제
